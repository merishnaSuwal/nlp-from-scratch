...
def naiveSoftmaxLossAndGradient(
    ...
    vc = centerWordVec
    uo = outsideVectors
    # Compute the softmax scores and probabilities
    y_hat = softmax(np.dot(vc, uo.T))
    y = 1
    ...
    delta = y_hat.copy()
    delta[outsideWordIdx] -= y  # y_hat - y
    gradCenterVec = np.dot(uo.T, delta)

    # Compute the gradient with respect to all the outside word vectors
    gradOutsideVecs = np.dot(delta[:, np.newaxis], vc[np.newaxis, :])
...
def negSamplingLossAndGradient(
    ...
    # Compute various quantities needed for the loss
    uo = outsideVectors
    vc = centerWordVec
    uo_vc = np.dot(uo[outsideWordIdx].T, vc)
    ...
    # Compute the gradient with respect to the center word vector
    gradCenterVec = -(1 - sigmoid(uo_vc)) * uo[outsideWordIdx] + np.sum(np.expand_dims(1 - sigmoid(-uk_vc), axis=1) * uk, axis=0)

    # Compute the gradient with respect to all the outside word vectors
    gradOutsideVecs = np.zeros(shape=uo.shape)
    gradOutsideVecs[outsideWordIdx] = -(1 - sigmoid(uo_vc)) * vc
    for i, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += (1 - sigmoid(-uk_vc)[i]) * vc
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ### YOUR CODE HERE (~8 Lines)
     # Iterate over outside words and compute loss and gradients
    v_c_idx = word2Ind[currentCenterWord]
    v_c = centerWordVectors[v_c_idx]
    
    u_o_words = outsideWords
    u_o = outsideVectors

    for outsideWord in u_o_words:
        outsideWordIdx = word2Ind[outsideWord]
        gradloss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(v_c, outsideWordIdx, u_o, dataset)
        loss += gradloss
        gradCenterVecs[v_c_idx] += gradCenterVec
